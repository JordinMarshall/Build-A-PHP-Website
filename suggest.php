...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if(!isset($error_message)) {
        ...
        $mail->SMTPSecure = 'tls';
        $mail->SMTPAuth = true;
        $mail->Username = "mixedbreedswag@gmail.com";
        $mail->Password = "ToRrEnTs1103";

        $mail->SetFrom($email, $name);
        $mail->Subject = "A Transactional Email From Web App";
        $mail->MsgHTML($email_body);
        $mail->AddAddress('mixedbreedswag@gmail.com', 'Jordin');


        $mail->isHTML(true);                                  // Set email format to HTML

        $mail->Subject = 'Catalog Suggestion from: ' . $name;
